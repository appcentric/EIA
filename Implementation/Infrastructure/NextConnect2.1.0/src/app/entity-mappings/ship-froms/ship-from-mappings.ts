...
@Component({
    ...
    templateUrl: './ship-from-mappings.html',
...
export class ShipFromMappingsComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this._service.getUrl(this.selectedAPI)
            .subscribe(
                shipFromMappings => { this.shipFromMappings = shipFromMappings },
    ...
    save(value: string) {
        if (this.newShipFromMapping){
            this._service.putShipFromMappingUrl(this.selectedAPI, this.form.value)
                .subscribe(
                    success => {        
                        this.loading = true;
                        this._service.getUrl(this.selectedAPI)
