...
<xsl:stylesheet version="2.0" xmlns:oraxsl="http://www.oracle.com/XSL/Transform/java"
                ...
                xmlns:ns2="http://xmlns.oracle.com/pcbpel/adapter/jms/DV1Application/NUC_SubscribeEBizPayment_JMS_CP_ABF/JMSConsumePayment"
  ...
  <oracle-xsl-mapper:schema>
    ...
    <!--GENERATED BY ORACLE XSL MAPPER 12.1.3.0.0(XSLT Build 140529.0700.0211) AT [FRI FEB 12 06:42:29 CST 2016].-->
  </oracle-xsl-mapper:schema>
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name = 'amountPaidDate' select="ns10:AmountPaidDate"> </xsl:variable>
             <xsl:variable name = 'startDate' select="$params/ns1:parameters/ns1:item/ns1:name"></xsl:variable>
              <xsl:variable name = 'endDate' select="$params/ns1:parameters/ns1:item/ns1:value"></xsl:variable>
       <!-- <start> <xsl:value-of select="$startDate"/></start>
               <amountpaiddate> <xsl:value-of select="$amountPaidDate"/></amountpaiddate>
               <end><xsl:value-of select="$endDate"/></end>-->
        <xsl:if test="((xsd:dateTime($startDate) &lt; xsd:dateTime($amountPaidDate)) and (xsd:dateTime($amountPaidDate) &lt; xsd:dateTime($endDate)))">
          <tns:Operation>
