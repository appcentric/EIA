...
<xsl:stylesheet version="1.0" xmlns:ns0="http://xmlns.eia.com/AO/Ebiz/D2S/InventoryTransactionAM/1.0"
            ...
            <xsl:if test=' dvm:lookupValue ("oramds:/apps/EIAMetadata/CommonArtifacts/dvm/EBS_QMOS_NSNE_ITEM_TYPE_MAP.dvm", "ITEMTYPE", ../../ns7:ItemType, "BUNDLE", "" ) = "Y" '>
              <xsl:if test='dvm:lookupValue ("oramds:/apps/EIAMetadata/CommonArtifacts/dvm/EBS_QMOS_TRANSACTION_TYPE_NAME_MAP.dvm", "TRANSACTION_TYPE_NAME", ../../ns7:TransactionType, "QMOS_BUNDLE", "" ) != "" and (not(../../ns7:TransactionType="WIP Completion" and ../../ns7:SubinventoryCode!="OSP"))'>
                ...
                <!-- <xsl:if test=' (../../ns7:ItemType = "NUC_HWY PROD") or  ((../../ns7:LPN != "") and ( substring (../../ns7:LPN, 1, 4 ) = "1201" or substring (../../ns7:LPN, 1, 4 ) = "1202" ))'>
                  ...
                  VERIFY WITH VIVEK
                  -->
                <!--xsl:if test=' (../../ns7:ItemType = "NUC_HWY PROD") or  ((../../ns7:LPN != "") and ( substring (../../ns7:LPN, 1, 3 ) = "120" or substring (../../ns7:LPN, 1, 3 ) = "120" ))'-->
                <tns:ApiBundleTransExt>
                  ...
                  <xsl:choose>
                    ...
                    <xsl:otherwise>
                      <!-- <xsl:when test="substring (../../ns7:LPN, 1, 4 ) = '1201'"> TEMP CHANGE DIKSHA-->
                      <!-- Updated based on the email from Qmos team Igor and Ebony and Ravi and Vivek Sub: RM lot question 8/22/2017 -->
                      <!--xsl:choose>
                        <xsl:when test="substring (../../ns7:LPN, 4, 1 ) = '8'">
                          <tns:millId>PA</tns:millId>
                        </xsl:when>
                        <xsl:otherwise>
                      ...
                      <xsl:call-template name="deriveMillId">
                        <xsl:with-param name="lpn" select="../../ns7:LPN"/>
                        <xsl:with-param name="resourceName" select="../../ns7:ResourceName"/>
                        <xsl:with-param name="transactionType" select="../../ns7:TransactionType"/>
                      </xsl:call-template>
                    </xsl:otherwise>
                  </xsl:choose>
                </tns:ApiBundleTransExt>
  ...
  <xsl:template name="deriveMillId">
    <xsl:param name="lpn"/>
    <xsl:param name="resourceName"/>
    <xsl:param name="transactionType"/>
    <xsl:choose>
      <xsl:when test="substring ($lpn, 1, 4 ) = '1201'">
        <tns:millId>PA</tns:millId>
      </xsl:when>
      <xsl:when test="substring ($lpn, 1, 4 ) = '1202'">
        <tns:millId>PB</tns:millId>
      </xsl:when>
      <xsl:when test="substring ($lpn, 1, 2 ) = 'PL' and substring ($lpn, 5, 1 ) = '1'">
        <tns:millId>PA</tns:millId>
      </xsl:when>
      <xsl:when test="substring ($lpn, 1, 2 ) = 'PL' and substring ($lpn, 5, 1 ) = '2'">
        <tns:millId>PB</tns:millId>
      </xsl:when>
      <xsl:when test="contains($resourceName,'RM1') and contains($transactionType,'Receipt')">
        <tns:millId>PA</tns:millId>
      </xsl:when>
      <xsl:when test="contains($resourceName,'RM2') and contains($transactionType,'Receipt')">
        <tns:millId>PB</tns:millId>
      </xsl:when>
      <xsl:otherwise>
        <tns:millId>PB</tns:millId>
