...
@IF [%1] == [] (@GOTO :SAR_FILE_MISSING)

@ SET ANT_ARGUMENTS=""

@IF NOT [%1] == [] (@ SET ANT_ARGUMENTS=-DsarFile=%1)
@IF NOT [%2] == [] (@ SET ANT_ARGUMENTS=%ANT_ARGUMENTS% -Drevision=%2)
@IF NOT [%3] == [] (@ SET ANT_ARGUMENTS=%ANT_ARGUMENTS% -DdeployPlan=%3)
@IF NOT [%4] == [] (@ SET ANT_ARGUMENTS=%ANT_ARGUMENTS% -Dpartition=%4)
...
%ANT_HOME%\bin\ant -f %EIA_HOME%\config\EIABuild.xml deploy-package  %ANT_ARGUMENTS%
...
:SAR_FILE_MISSING
@echo ****************** ERORR ERROR ERROR ****************** 
@ECHO SAR File parameter value is not passed. Please refer the Deploy_ReadMe.txt for the syntax
@echo ****************** ERORR ERROR ERROR ******************
%ANT_HOME%\bin\ant -f %EIA_HOME%\config\EIABuild.xml help-deploy-package
@echo There are some errors in the deployment.               
@SHIFT 
@GOTO :ENDFILE
