...
@IF NOT [%3] == [] (@ SET ANT_ARGUMENTS=%ANT_ARGUMENTS% -Drevision=%3)
@IF NOT [%4] == [] (@ SET ANT_ARGUMENTS=%ANT_ARGUMENTS% -DdeployPlan=%4)
@IF NOT [%5] == [] (@ SET ANT_ARGUMENTS=%ANT_ARGUMENTS% -Dpartition=%5)

@set PATH=.:%PATH%;%JAVA_HOME%/bin;%ANT_HOME%/bin

@set CLASSPATH=.;%EIA_HOME%/lib/EIAUtils.jar;%ANT_HOME%/lib;%CLASSPATH%

%ANT_HOME%\bin\ant -f %EIA_HOME%\config\EIABuild.xml deploy %ANT_ARGUMENTS%
...
@echo ****************** ERORR ERROR ERROR ******************
%ANT_HOME%\bin\ant -f %EIA_HOME%\config\EIABuild.xml help-deploy
@echo There are some errors in the deployment.               
@SHIFT 
@GOTO :ENDFILE
